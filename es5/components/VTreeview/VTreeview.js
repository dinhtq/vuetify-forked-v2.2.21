...
/* @vue/component */
  ...
  created: function created() {
    ...
    this.value.map(getValue).forEach(function (value) {
      return _this2.updateSelected(value, true, true);
    });
    this.active.map(getValue).forEach(function (value) {
      return _this2.updateActive(value, true);
    });
  },
  ...
  methods: {
    ...
    calculateState: function calculateState(node, state) {
      var counts = state[node].children.reduce(function (counts, child) {
        counts[0] += +Boolean(state[child].isSelected);
        counts[1] += +Boolean(state[child].isIndeterminate);
        return counts;
      }, [0, 0]);
      var isSelected = !!state[node].children.length && counts[0] === state[node].children.length;
    ...
    updateSelected: function updateSelected(key, isSelected) {
      ...
      if (this.selectionType !== 'independent') {
        this.getDescendants(key).forEach(function (descendant) {
          if (!(0, _helpers.getObjectValueByPath)(_this9.nodes[descendant].item, _this9.itemDisabled) || isForced) {
        ...
        parents.forEach(function (parent) {
          var calculated = _this9.calculateState(parent, _this9.nodes);

          _this9.nodes[parent].isSelected = calculated.isSelected;
          _this9.nodes[parent].isIndeterminate = calculated.isIndeterminate;
